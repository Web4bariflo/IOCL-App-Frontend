...
export default function ContactorSensorScreen() {
  ...
  const fetchDetectionLogs = async () => {
    ...
    try {
      ...
      console.log('Detection Logs Response:', response);
    ...
    useCallback(() => {
      const loadSensors = async () => {
        try {
          ...
          // API returns sensors in response.sensors
          const sensorData = response?.sensors || [];
          ...
          // Count sensors whose current_state is ON
          const onlineSensorCount = sensorData.filter(
            (sensor: any) => sensor.current_state === 'ON'
          ).length;

          setActiveSensorCount(onlineSensorCount);

          console.log('Total Sensors:', sensorData.length);
          console.log('Online Sensors:', onlineSensorCount);

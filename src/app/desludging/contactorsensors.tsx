...
import { getSensors, getEquipmentManualLogs } from '../../api/flocculationApi';
...
export default function ContactorSensorScreen() {
  ...
  const fetchSensors = async () => {
    try {
      ...
      if (!stageId) {
        console.log('desludging Stage ID not found');
        return;
      }
      ...
      if (response?.success) {
        ...
        setSensorCount(response?.sensor_count || 0);
      }
    } catch (error) {
  ...
  const onlineSensorCount = sensors.filter(
    sensor => sensor.current_state === 'ON'
  ).length;

  const allSensorsOnline =
    sensorCount > 0 && onlineSensorCount === sensorCount;

  const totalSensorCount = sensors.length;
  ...
  const fetchSensorLogs = async () => {
    try {
      ...
      if (!stageId || !storedSensorIds) {
        console.log('Stage ID or Sensor IDs not found');
        return;
      }

      const sensorIds = JSON.parse(storedSensorIds);

      // Sensor 1 -> sensorIds[0] -> 23
      // Sensor 2 -> sensorIds[1] -> 24
      const equipmentId =
        activeTab === 'SENSOR 1'
          ? sensorIds[0]
          : sensorIds[1];
      ...
      console.log('Sensor Logs Response:', response);
      ...
      console.log('Failed to fetch sensor logs:', error);
    }
  };
            ...
            <View style={styles.statusTextContainer}>
              ...
              <Text style={styles.statusSubtitle}>
                {onlineSensorCount === totalSensorCount
                  ? `${totalSensorCount} sensors online`
                  : `${totalSensorCount} sensors offline`}
              </Text>
            </View>
